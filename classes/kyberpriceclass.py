...
class kyberprice:
	def main(source, destination, amount):
		...
		def getkyberprice(token1address, token2address, amount):
			expectedreturn = kyberexchangerate.functions.getExpectedRate(token1address, token2address, int(amount)).call({'from': baseaccount})[0]
			return expectedreturn

		worstRate = getkyberprice(tokenarray[source][0], tokenarray[destination][0], amount)
		output = str(amount) + " " + source + " after slippage will get " + str(worstRate) + " " + destination
		return worstRate
